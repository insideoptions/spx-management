...
const GridBackground = ({ 
  ...
  const themeDotColor = isDarkMode 
    ? `rgba(255, 255, 255, ${isMobile ? 0.15 : 0.12})` // White dots for dark theme
    : `rgba(0, 0, 0, ${isMobile ? 0.15 : 0.12})`; // Black dots for light theme

  // Force render immediately without waiting for hydration
  if (!mounted) {
    const fallbackDotColor = `rgba(255, 255, 255, ${isMobile ? 0.15 : 0.12})`;
    return (
      <div className="absolute inset-0 min-h-screen overflow-hidden" style={{ backgroundColor }}>
        <div 
          className={`absolute inset-0 ${isMobile ? 'opacity-30' : 'opacity-25'}`}
          style={{
        ...
        />
        <div 
          className={`absolute inset-0 ${isMobile ? 'opacity-15' : 'opacity-12'}`}
          style={{
  ...
  return (
    <div className="absolute inset-0 min-h-screen overflow-hidden" style={{ backgroundColor }}>
      ...
      <div 
        className={`absolute inset-0 ${isMobile ? 'opacity-30' : 'opacity-25'} ${animated ? 'animate-pulse' : ''}`}
        style={{
          backgroundImage: `
